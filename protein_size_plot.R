...
ecoli_prots = read.table("~/git/genome-size/ecoli_uniprot.sizes", sep=" ")
ecoli_hist = hist(ecoli_prots[,2], breaks=seq(0,50000,50), plot=FALSE)
yeast_prots = read.table("~/git/genome-size/yeast_uniprot.sizes", sep=" ")
yeast_hist = hist(yeast_prots[,2], breaks=seq(0,50000,50), plot=FALSE)
human_prots = read.table("~/git/genome-size/human_uniprot.sizes", sep=" ")
